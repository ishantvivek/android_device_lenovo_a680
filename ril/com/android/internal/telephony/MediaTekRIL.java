...
/**
 * Custom MTK6589 RIL cm10.2 by shutt1e - backport cm11 Omnirom mediatekRIL
 *
 * {@hide}
 */
...
// fin qui nulla di che
            ...
            if ((lacValid == true) && (strings[i] != null)) {
                ...
                if(lac != "") {
                    ...
                    if(lacValue != 0xfffe) {
                        ...
                        if(sEons != null) {
                            strings[i] = sEons;           
                            Log.d(LOG_TAG, "plmn name update to Eons: "+strings[i]);
                        }*/
                    } else {
		...
		// original String temp = init.unOptimizedOperatorReplace(strings[i+2]);
		String temp = init.operatorReplace(strings[i+2]);		
...
//
    ...
    responseCrssNotification(Parcel p) {
        ...
        Log.e(LOG_TAG, "NOT PROCESSING CRSS NOTIFICATION");
        return null;
    }
    ...
    private Object responseEtwsNotification(Parcel p) {
        ...
        return response;*/
        Log.e(LOG_TAG, "NOT PROCESSING ETWS NOTIFICATION");
...
// absent in cm10.2  case RIL_REQUEST_ENTER_NETWORK_DEPERSONALIZATION: ret =  responseInts(p); break;
    ...
    processUnsolicited (Parcel p) {
		...
		break;
            default:
                Log.i(LOG_TAG, "Unprocessed unsolicited known MTK response: " + response);
        }

        if (rewindAndReplace) {
            Log.w(LOG_TAG, "Rewriting MTK unsolicited response to " + newResponseCode);
    ...
    responseOperator(Parcel p) {
	    ...
	    Operators init = new Operators ();
	    //original String temp = init.unOptimizedOperatorReplace(response[2]);
	    String temp = init.operatorReplace(response[2]);
	...
	SignalStrength s = new SignalStrength(p);	
...
// this need a patch in RIL.java "protected integer mInstanceId;"
